...
def transpose_vol(ct, skin, vol_id=""):

    max_dim = np.argmax(ct.shape)
    ...
    if max_dim != 1:
        ...
        transpose_list = [a[0], max_dim, a[1]]
        ct = np.transpose(ct, transpose_list)
        skin = np.transpose(skin, transpose_list)

        a = np.count_nonzero(np.sum(skin, axis=0))
        b = np.count_nonzero(np.sum(skin, axis=-1))

        if a > b:
            ct = np.transpose(ct, [2, 1, 0])
            skin = np.transpose(skin, [2, 1, 0])
