...
class ProbeTilting(us.BaseMethod):
    ...
    def execute(self, image, label_full, xy_probe=(0, 0), alpha_probe=0):

        sharp_label = self._get_biggest_component(label_full)
        label = cv2.dilate(sharp_label, self.dilation_kernel, iterations=1)  # TODO maybe delete?
        ...
        bone_probe_distance = self._get_bone2probe_distance(xy_probe, x_sorted, p, self.plot_result)
        ...
        blur_label = self._blur(sharp_label, kernel=self.label_blur_kernel, sigma=self.label_blur_sigma)

        augmented_image = blur_label * (image*intensity_rescaling_mask) + (1 - blur_label) * image
        shadow_mask = self._get_shadow_mask(sharp_label, xy_probe)
