...
class TestNakagamiNoise(unittest.TestCase):
    ...
    def test_execute(self):
        ...
        method = us.NakagamiNoise()

        for _ in range(10):
            nakagami_shape_parameter = float(random.randint(0, 100))/100 + 0.001

            noise_image, noise_label = method.execute(image=image,
                                                      label=label,
                                                      nakagami_shape_parameter=nakagami_shape_parameter)

            self.plot(image, label, noise_image, noise_label)
